...
namespace Tetris
{
    public class Brick : Square
    {
        ...
        public Brick(Texture2D Texture, Vector2 Position) : base(Texture, Position)
        {
            ...
            sideMoveTimer = 0.5f;
            sideMoveCount = 0;
            Rectangle.X = (int)Position.X/32 * 32; Rectangle.Y = (int)Position.Y/32 * 32;
            alive = true;
            coliding = false;
        }
        ...
        public void Move(Square[,] PlayField)
        {
        ...
        {
            if (Rectangle.Bottom >= Globals.WindowSize.Y) { coliding = true; PlayField[Rectangle.X / 32, Rectangle.Y / 32].ocupied = true; }          
        }



        public void CheckRectColision(Rectangle rect, Square[,] PlayField  )

        {
            if (Rectangle.Bottom == rect.Top && Rectangle.X == rect.X)
            {
                coliding = true;
                PlayField[Rectangle.X / 32, Rectangle.Y/32].ocupied = true;
            }
            
        }


        public void Update(Square[,] PlayField)
        {
            if (!coliding) Fall();          
            else { alive = false; }
